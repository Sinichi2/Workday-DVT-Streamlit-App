...
def main():
    ...
    # Write two sheets so Dashboard can render full Condition + Description on
    # the By Failure Reason panel without needing a separate rules upload.
    with pd.ExcelWriter(validated_path, engine="openpyxl") as w:
        validated_df.to_excel(w, sheet_name="Validated", index=False)
