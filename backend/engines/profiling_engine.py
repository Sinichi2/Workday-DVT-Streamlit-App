...
def profile_dataset(df):
    ...
    # Anomaly flags. Each issue carries `row_indices` so the UI can show the
    # actual rows that triggered it. For column-level issues with no specific
    # bad rows, row_indices is left empty.
    issues = []
    for col in df.columns:
        series = df[col]
        non_blank_mask = ~series.apply(_is_blank)
        ...
        blank_indices = list(df.index[~non_blank_mask])
        ...
        if non_blank > 0 and distinct == 1:
            issues.append({
                ...
                "row_indices": [],  # affects every row trivially; not useful to list
            })

        if _has_leading_or_trailing_whitespace(series):
            ws_mask = series.apply(
                lambda v: isinstance(v, str) and v != v.strip() and v.strip() != ""
            )
            issues.append({
                "Column": col, "Severity": "Low",
                "Issue": "Whitespace in values",
                "Detail": "One or more values have leading or trailing whitespace.",
                "row_indices": list(df.index[ws_mask]),
            })

        if _has_mixed_case_inconsistency(series):
            # Flag rows whose value is not the most-common casing of its lower form.
            mc_indices = []
            if pd.api.types.is_object_dtype(series):
                from collections import Counter as _C
                # canonical casing per lower-form = most common casing
                groups = {}
                for v in series:
                    if _is_blank(v) or not isinstance(v, str):
                        continue
                    s = v.strip()
                    groups.setdefault(s.lower(), _C())[s] += 1
                canonical = {k: c.most_common(1)[0][0] for k, c in groups.items() if c}
                for i, v in zip(df.index, series):
                    if _is_blank(v) or not isinstance(v, str):
                        continue
                    s = v.strip()
                    if canonical.get(s.lower()) and s != canonical[s.lower()]:
                        mc_indices.append(i)
            issues.append({
                "Column": col, "Severity": "Low",
                "Issue": "Mixed casing",
                "Detail": "The same value appears in multiple casings.",
                "row_indices": mc_indices,
            })

    if dup_count > 0:
        # All rows that are part of any duplicate group (keep all copies)
        dup_indices = list(df.index[df.duplicated(keep=False)])
        issues.append({
            "Column": "(entire row)",
            "Severity": "High",
    ...
    # Build the displayable DataFrame (drop row_indices from display, keep in metadata)
    if issues:
        issues_df = pd.DataFrame([
            {k: v for k, v in iss.items() if k != "row_indices"}
            for iss in issues
        ])
    else:
        issues_df = pd.DataFrame(columns=["Column", "Severity", "Issue", "Detail"])

    # Attach the row_indices list to the DataFrame as metadata via a parallel list,
    # since storing lists in cells works but pandas operations would mangle them.
    issues_df.attrs["row_indices_by_issue"] = [iss.get("row_indices", []) for iss in issues]
